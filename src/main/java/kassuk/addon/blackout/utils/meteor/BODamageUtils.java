...
public class BODamageUtils {
    ...
    public static double anchorDamage(LivingEntity player, Vec3d anchor) {
        if (mc.world.getBlockState(new BlockPos(tympanicUtils.vec3dToVec3i(anchor))).getBlock().equals(Blocks.RESPAWN_ANCHOR)) {
            mc.world.removeBlock(new BlockPos(tympanicUtils.vec3dToVec3i(anchor)), false);
            double damage = bedDamage(player, anchor);
            mc.world.setBlockState(new BlockPos(tympanicUtils.vec3dToVec3i(anchor)), Blocks.RESPAWN_ANCHOR.getDefaultState());
            return damage;
        }
    ...
    private static double normalProtReduction(Entity player, double damage) {
        int protLevel = EnchantmentHelper.getProtectionAmount(player.getArmorItems(), player.getDamageSources().generic());
    ...
    private static double blastProtReduction(Entity player, double damage, Explosion explosion) {
        int protLevel = EnchantmentHelper.getProtectionAmount(player.getArmorItems(), player.getDamageSources().explosion(explosion));
        if (protLevel > 20) protLevel = 20;
    ...
    public static BlockHitResult raycast(RaycastContext context) {
        return BlockView.raycast(context.getStart(), context.getEnd(), context, (raycastContext, blockPos) -> {
            ...
            return BlockHitResult.createMissed(raycastContext.getEnd(), Direction.getFacing(vec3d.x, vec3d.y, vec3d.z), new BlockPos(tympanicUtils.vec3dToVec3i(raycastContext.getEnd())));
        });
    }
