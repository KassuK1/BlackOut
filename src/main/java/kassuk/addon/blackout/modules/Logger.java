...
public class Logger extends BlackOutModule {
    ...
    private String packetMessage(Packet<?> packet) {
        ...
        if (packet instanceof HandshakeC2SPacket p) return "Handshake state:" + p.getIntendedState().name() + " address:" + p.getAddress() + " protocol:" + p.getProtocolVersion();

        if (packet instanceof LoginHelloC2SPacket p) return "LoginHello id:" + p.profileId().get() + " name:" + p.name();
        if (packet instanceof LoginKeyC2SPacket) return "LoginKey";
        if (packet instanceof LoginQueryResponseC2SPacket p) return "LoginQueryResponse id:" + p.getQueryId();

        if (packet instanceof AdvancementTabC2SPacket p) return "AdvancementTab action:" + p.getAction() + " toOpen:" + p.getTabToOpen().toString();
        ...
        if (packet instanceof BookUpdateC2SPacket p) return "BookUpdate slot:" + p.getSlot() + " pages:" + p.getPages().size() + " title:" + p.getTitle().toString();
        ...
        if (packet instanceof ClientSettingsC2SPacket p) return "ClientSettings language:" + p.language() + " allowsListing:" + p.allowsListing() + " chatColors:" + p.chatColors() + " chatVisibility:" + p.chatVisibility().name() + " filterText:" + p.filterText() + " mainArm:" + p.mainArm().name() + " playerModelBitMask:" + p.playerModelBitMask() + " viewDistance:" + p.viewDistance();
        ...
        if (packet instanceof CreativeInventoryActionC2SPacket p) return "CreativeInventoryAction slot:" + p.getSlot() + " item:" + p.getItemStack().getItem() + " count:" + p.getItemStack().getCount();
        if (packet instanceof CustomPayloadC2SPacket p) return "CustomPayload data:" + p.getData() + " channel:" + p.getChannel();
        ...
        if (packet instanceof PlayerInteractEntityC2SPacket p) return "PlayerInteractEntity id:" + ((MixinInteractEntityC2SPacket) p).getId();
        ...
        if (packet instanceof PlayerSessionC2SPacket p) return "PlayerSession sessionId:" + p.chatSession().sessionId().toString();
        if (packet instanceof PlayPongC2SPacket p) return "PlayPong parameter:" + p.getParameter();
        ...
        if (packet instanceof RecipeBookDataC2SPacket p) return "RecipeBookData identifier:" + p.getRecipeId().toString();
        ...
        if (packet instanceof SpectatorTeleportC2SPacket p) return "SpectatorTeleport UUID:" + ((MixinSpectatorTeleportC2SPacket) p).getID().toString();
        ...
        if (packet instanceof UpdateCommandBlockC2SPacket p) return "UpdateCommandBlock x:" + p.getBlockPos().getX() + " y:" + p.getBlockPos().getY() + " z:" + p.getBlockPos().getZ() + " command:" + p.getCommand() + " type:" + p.getType().name() + " alwaysActive:" + p.isAlwaysActive() + " conditional:" + p.isConditional() + " trackOutput:" + p.shouldTrackOutput();
        ...
        if (packet instanceof UpdateJigsawC2SPacket p) return "UpdateJigsaw x:" + p.getPos().getX() + " y:" + p.getPos().getY() + " z:" + p.getPos().getZ() + " name:" + p.getName().toString() + " finalState:" + p.getFinalState() + " joint:" + p.getJointType().name() + " pool:" + p.getPool().toString() + " target:" + p.getTarget().toString();
        ...
        if (packet instanceof EntityS2CPacket p) return "Entity id:" + ((MixinEntityS2CPacket) p).getId() + " deltaX:" + p.getDeltaX() + " deltaY:" + p.getDeltaY() + " deltaZ:" + p.getDeltaZ() + " yaw:" + p.getYaw() + " pitch:" + p.getPitch() + " hasRotation:" + p.hasRotation() + " posChange:" + p.isPositionChanged() + " ground:" + p.isOnGround();
        if (packet instanceof EntitySetHeadYawS2CPacket p) return "EntitySetHeadYaw id:" + ((MixinEntitySetHeadYawS2CPacket) p).getId();
        ...
        if (packet instanceof EntityStatusS2CPacket p) return "EntityStatus status:" + p.getStatus() + " id:" + ((MixinEntityStatusS2CPacket) p).getId();
        ...
        if (packet instanceof PlayerRespawnS2CPacket p) return "PlayerRespawn gameMode:" + p.getGameMode().getName() + " prevGameMode:" + p.getPreviousGameMode().getName() + (!p.getLastDeathPos().isPresent() ? " <deathPosNotPresent>" : " deathX:" + p.getLastDeathPos().get().getPos().getX() + " deathY:" + p.getLastDeathPos().get().getPos().getY() + " deathZ:" + p.getLastDeathPos().get().getPos().getZ()) + " portalCooldown:" + p.getPortalCooldown() + " debugWorld:" + p.isDebugWorld() + " flatWorld:" + p.isFlatWorld();
        ...
        if (packet instanceof SetCameraEntityS2CPacket p) return "SetCameraEntity id:" + ((MixinSetCameraEntityS2CPacket) p).getId();
        ...
        if (packet instanceof StatisticsS2CPacket p) return "Statistics" + stats(p.getStatMap());
    ...
    private String recipes(List<Recipe<?>> list) {
        StringBuilder builder = new StringBuilder();
        for (Recipe<?> r : list) {
            builder.append(r.createIcon().getItem().getName()).append("\n");
        }
        return builder.toString();
    }
